...
for chl in chlaruns:
    ...
    #print shape as a failsafe that it has run properlyy
    poc = poc.drop_duplicates()
    print(poc.shape)

    #save as a csv
    poc.to_csv(f"{fp}input_data/merged_POC_all_{chl}_1103.csv", index = False)

    #percentage in each method category
    perc_method = pd.DataFrame(data = {"Method":[],
                            "Percentage":[],
                            "Count":[]})

    for m in set(poc["Method"]):
        subset = poc[poc["Method"] == m]
        perc = subset.shape[0]/poc.shape[0]*100
        perc_method.loc[len(perc_method)] = [m,perc, subset.shape[0]]
    perc_method.to_csv(f"{fp}input_data/perc_method_merged_POC_all_{chl}_1103.csv", index = False)